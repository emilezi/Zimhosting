<?php
    ...
    <title>".$titre_header_compte_data_image."</title>
    </head>";
   ...
   echo "<h1><center>".$titre_header_compte_data_image."</center></h1>";
   ...
   echo "<div id='compte-element'>";
	...
	<h2><?=$titre_compte_data_new_image?></h2>
	<hr/><br/>
	...
	if ($nb_sujets == 0) {
	echo "<h2>".$titre_compte_data_image."</h2>";
	echo "<hr/>";
	echo "<p>".$message_data_image_undefined_2."</p>";
	}
	else{
	...
	echo "<img src=".$result['data_link']." class='picture' width='250px' onclick='ImageViewer".$i."();'>";

	echo "
	<script>
	function ImageViewer".$i."(){
		var div = document.getElementById('box-data');
		div.innerHTML += '<div id=data-viewer><div id=data-content><img src=".$result['data_link']." width=450px></div><div class=data-viewer-icon><a href=del_data.php?fichier_a_supprimer=".$result['id_data']."><img src=../img/icones/trash.png style=padding:15px;></a><img src=../img/icones/close.png style=padding:15px; onclick=ViewerClose();></div></div>';
		}
	</script>
	";
	...
	echo "</div>";
		...
		echo "<h2>".$titre_compte_information."</h2>";
		echo "<hr/>";
		echo "<p>".$message_data_compte_undefined_1."</p>";
		echo "</div>";
	}
